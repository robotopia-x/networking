...
function PeerStar (opts) {
  ...
  function startUp () {
    ...
    if (opts.isMain) {
      self.hub.subscribe(MAIN).once('open', connect.bind(null, self, self.hub)).pipe(through.obj(function (data, enc, cb) {
        data = self.unwrap(data, MAIN)
        if (self.closed || !data) return cb()

        var peer = self.contacts[data.from]
        if (!peer) {
          if (!data.signal || data.signal.type !== 'offer') {
            debug('skipping non-offer', MAIN)
            return cb()
          }

          debug('connecting to new peer: ' + data.from, MAIN)
          peer = new SimplePeer({
            wrtc: self.wrtc,
            channelConfig: self.channelConfig,
            config: self.config,
        ...
        debug('signalling to ' + data.from + ':' + data.signal, MAIN)
  ...
  function connect (star) {
    if (star.closed) return
    var data = {type: 'connect', from: self.CID}
    data = star.wrap(data, ALL)
    star.hub.broadcast(ALL, data, function () {
      setTimeout(connect.bind(null, star, star.hub), Math.floor(Math.random() * 2000) + 6000)
    })
  }
